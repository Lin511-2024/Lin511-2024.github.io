...
We'll be using a program called `pyenv` to manage the versions of python we use. To install `pyenv`, open the VSCode Command Palette (with {{< kbd mac=Shift+Command+P windows=Shift+Control+P linux=Shift+Control+P >}}) which will open the command line interface. Then,

-   [ ] copy this line of code code from this page ({{< kbd mac=Command+C windows=Control+C, linux=Control+C >}})

``` bash
curl https://pyenv.run | bash
```

-   [ ] Paste it into the terminal (({{< kbd mac=Command+V windows=Control+V, linux=Control+V >}})
-   [ ] Hit enter.
